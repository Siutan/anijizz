...
export default function NavigationItem({ name, address, icon }: NavigationItemProps): React.ReactElement {
  ...
  return (
    ...
    >
      <p className={isActive ? "text-teal-500 dark:text-teal-500" : "text-gray-600 dark:text-gray-100"}> {icon} </p>
      <p className={"w-full text-[14px] capitalize text-inherit " + (isActive ? "font-semibold text-teal-500 dark:text-teal-500" : "font-normal dark:text-gray-100")}> {name} </p>
      <div className="relative">
        <div className={"w-1 rounded-full bg-teal-500 z-10 absolute -translate-x-[1px] " + (isActive ? "transition-all duration-200 h-4 translate-y-0" : "h-0 translate-y-2")} />
      </div>
    </Link>
  )
}