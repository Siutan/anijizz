...
export default function Content(props: Props) {
  ...
  const nativeTitle = (selectedAnime?.title?.native !== title ? selectedAnime?.title?.native?.replace(/[A-Za-z0-9]/g, "") : selectedAnime?.studios?.nodes![0]?.name + "'s")!;
    ...
    <>
      ...
      >
        ...
        <div className="w-full flex justify-between">
          ...
          <p className="flex items-center justify-center space-x-1 px-4 py-1 text-sm text-white medium bg-teal-600 rounded-full">
        ...
        <div className="flex flex-col justify-end w-full h-full overflow-hidden md:max-w-[80%] lg:max-w-[50%] md:self-start space-y-2 md:space-y-3">
          <h1 className="max-w-[76%] lg:max-w-[60%] text-2xl lg:text-xl overflow-hidden whitespace-nowrap text-ellipsis text-teal-600 font-semibold"> {nativeTitle} </h1>
          <h1 className="text-xl lg:text-2xl font-medium text-white max-w-[100%] lg:max-w-[100%]"> {title} </h1>
          <p
        ...
        <div className="w-full flex justify-between md:justify-end space-x-3 absolute md:bottom -translate-y-4 md:-translate-y-0-0 md:bottom-0 p-4 md:px-6">
          <button
            ...
            className={"py-1 px-1 rounded-full bg-whiteAlpha opacity-20 hover:opacity-100 md:opacity-100 disabled:opacity-30 disabled:cursor-not-allowed transition-all"}
          >
